...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Widget _menuImage(MenuItemModel item) {
    ...
    if (hasUrl) {
      return Image.network(
        ...
        errorBuilder: (_, __, ___) {
          ...
          final fallback = ImageResolver.getMenuImage(category, item.itemName);
          return Image.asset(
            fallback,
            height: 110,
            width: 110,
    ...
    // Fallback to local asset resolver
    final category = item.itemCuisine.toLowerCase().replaceAll(" ", "_");
    final fallback = ImageResolver.getMenuImage(category, item.itemName);
  ...
  Widget _buildMenuCard(MenuItemModel item, ThemeData theme) {
    final imageWidget = _menuImage(item);
