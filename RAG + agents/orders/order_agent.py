...
class OrderAgent:
    ...
    def save_and_summarize_order(self, cart_id: str, cart_summary: Dict) -> Dict:
        """
        Saves the final order to the 'orders' table,
        inserts snapshot rows into 'order_items',
        and prepares a confirmation summary.
        """
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                # 2) Insert snapshot rows into order_items
                for item in cart_summary.get("items", []):
                    cur.execute(
                        """
            ...
            # 3) Commit once after everything succeeds
            conn.commit()
