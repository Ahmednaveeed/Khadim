...
class CartTools:
    ...
    def _init_tables(self):
        """Initialize cart tables in Postgres (aligned with DB dump)"""
        with self.db.get_connection() as conn:
            with conn.cursor() as cur:
                # Ensure columns exist (safe in case tables already exist)
                cur.execute("""
                ...
                # In case cart existed without user_id in older environments
                cur.execute("""
                    ALTER TABLE cart
    ...
    def create_cart(self, user_id: str) -> Dict:
        """
        Create or reuse ONE active cart for the user.
        cart_id is its own UUID (NOT equal to user_id).
        """
        with self.db.get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:

                # 1) Reuse existing active cart
                cur.execute("""
                    SELECT cart_id
                    FROM cart
                ...
                if row:
                    return {
                        ...
                        "message": "Active cart reused"
                    }

                # 2) Create new cart
                cart_id = str(uuid.uuid4())
                cur.execute("""
                    INSERT INTO cart (cart_id, user_id, status, updated_at)
